...
void act_move(int act, float act_pos, int table_stop) {
	if (table_stop == 1) {
		...
		dutyS[act] = (int) (pos_diff[act] * Kp[act] + Ki[act] * integral
				+ Kd[act] * derivative);
		...
		if (act_pos > (posFP[act] + sseLA)) {
			switch (act) {
			case 0:
				linAct1(dutyS[act], periodS[act], 2);
				break;

			case 1:
				linAct2(dutyS[act], periodS[act], 2);
				break;

			case 2:
				linAct3(dutyS[act], periodS[act], 2);
				break;

			case 3:
				linAct4(dutyS[act], periodS[act], 2);
				break;
			}
		} else if (act_pos < (posFP[act] - sseLA)) {
			switch (act) {
			case 0:
				linAct1(dutyS[act], periodS[act], 1);
				break;

			case 1:
				linAct2(dutyS[act], periodS[act], 1);
				break;

			case 2:
				linAct3(dutyS[act], periodS[act], 1);
				break;

			case 3:
				linAct4(dutyS[act], periodS[act], 1);
				break;
			}
		} else if ((posFP[act] < (act_pos + sseLA))
				&& (posFP[act] > (act_pos - sseLA))) {
			switch (act) {
			case 0:
				linAct1(dutyS[act], periodS[act], 0);
				break;

			case 1:
				linAct2(dutyS[act], periodS[act], 0);
				break;

			case 2:
				linAct3(dutyS[act], periodS[act], 0);
				break;

			case 3:
				linAct4(dutyS[act], periodS[act], 0);
				break;
			}
...
void sensUSART_puts(USART_TypeDef *USARTx, volatile int num) {
	while (USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET)
		; //until TC flag is one !(USARTx -> SR & 0x040)
...
void dcPosition(float positionSet) {
	...
	if (((errorDC < 0) && (errorDC > -ssError))
			|| ((errorDC > 0) && (errorDC < ssError))) { // Steady state error
		dcMove(dutyDC, 0);
		delay_ms(50);
	} else {
		if ((errorDC < 0) && (errorDC <= (-ssError))) {
...
void initOneMeasure(int fan) {
	...
	while (init_t < 100) {
		if ((sensorFeedback[0] == initCommand[0])
				&& (sensorFeedback[1] == initCommand[1])) {
			sensConnected = 1;
		}
...
void readSensVal() {
	...
	int measureOK = 0;
	delay_ms(100);
	if ((sensorFeedback[0] == readCommand[0])
			&& (sensorFeedback[1] == readCommand[1])) {
		measureOK = 1;
	}
	if (measureOK) {
 ...
 */
void modemFeedbackCheck(volatile char *cmdAT, volatile char *okAT,
		volatile char *errAT) {
	at_ok = 0;
	at_err = 0;
	//PoÅ¡iljanje ukaza
